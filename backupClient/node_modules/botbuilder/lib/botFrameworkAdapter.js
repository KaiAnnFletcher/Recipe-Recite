...
class BotFrameworkAdapter extends botbuilder_core_1.BotAdapter {
    ...
    async getUserToken(context, connectionName, magicCode) {
        ...
        const result = await client.userToken.getToken(userId, connectionName, { code: magicCode, channelId: context.activity.channelId });
     ...
     * */
    async getTokenStatus(context, userId, includeFilter) {
     ...
     * @param connectionName Name of the auth connection to use.
     */
    async getAadTokens(context, connectionName, resourceUrls) {
        if (!context.activity.from || !context.activity.from.id) {
     ...
     */
    async emulateOAuthCards(contextOrServiceUrl, emulate) {
        this.isEmulatingOAuthCards = emulate;
        const url = this.oauthApiUrl(contextOrServiceUrl);
        await botframework_connector_1.EmulatorApiClient.emulateOAuthCards(this.credentials, url, emulate);
    }
    /**
            ...
            switch (activity.type) {
                ...
                default:
                    ...
                    if (activity.type === 'trace' && activity.channelId !== 'emulator') {
                        ...
                        responses.push(await client.conversations.replyToActivity(activity.conversation.id, activity.replyToId, activity));
                    }
                    else {
                        responses.push(await client.conversations.sendToConversation(activity.conversation.id, activity));
                    }
                    break;
            }
        }
    ...
    async updateActivity(context, activity) {
        ...
        const client = this.createConnectorClient(activity.serviceUrl);
        await client.conversations.updateActivity(activity.conversation.id, activity.id, activity);
    }
    /**
     ...
     */
    async authenticateRequest(request, authHeader) {
        const claims = await botframework_connector_1.JwtTokenValidation.authenticateRequest(request, authHeader, this.credentialsProvider, this.settings.channelService);
